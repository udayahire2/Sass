      ...
      <FramePanel className="p-0">
        {materialsLoading ? (
          ...
          <>
            ...
            <div className="block md:hidden p-4 space-y-3">
              {userMaterials.map((m) => {
                ...
                return (
                  <div
                    key={m._id || m.id}
                    className="rounded-xl border border-border/50 bg-card p-4 shadow-sm transition-all hover:shadow-md"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                          <span className="inline-flex items-center rounded border border-border/50 bg-secondary px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                            {m.type}
                          </span>
                          <span>•</span>
                          <span>{m.subject}</span>
                      ...
                      <Badge className={cn("rounded-full px-2.5 py-1 text-[11px] font-semibold tracking-wide border shadow-sm flex items-center gap-1.5 shrink-0 ml-2", status.colorClass)}>
                        {status.animate && (
            ...
            {/* Desktop Table Layout – hidden on mobile, shown on md+ */}
