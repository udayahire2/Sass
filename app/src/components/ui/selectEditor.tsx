...
export const selectTriggerVariants = cva(
  // Changed: Removed w-full, min-w-36, and justify-between. Added w-fit and justify-start to hug content tightly.
  "relative inline-flex min-h-9 w-fit select-none items-center justify-start gap-2 rounded-lg border-none bg-transparent px-[calc(--spacing(3)-1px)] text-left text-base text-foreground outline-none ring-ring/24 transition-shadow focus-visible:ring-[3px] data-disabled:pointer-events-none data-disabled:opacity-64 sm:min-h-8 sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    defaultVariants: {
      size: "default",
    },
...
export function SelectButton({
  ...
  const defaultProps = {
    children: (
      <>
        {/* Changed: Removed flex-1 here so the text doesn't push the icon away */}
        <span className="truncate in-data-placeholder:text-muted-foreground/72">
          {children}
        </span>
...
export function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      {...props}
    >
      {/* Ensure any raw text passed directly also doesn't push elements around unintentionally */}
...
export function SelectValue({
  ...
  return (
    <SelectPrimitive.Value
      className={cn(
        // Changed: Removed flex-1 here as well
